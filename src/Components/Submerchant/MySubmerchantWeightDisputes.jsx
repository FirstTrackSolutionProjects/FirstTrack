...
import ViewWeightDisputePopup from "@/Components/ViewWeightDisputePopup";
...
const Listing = () => {
  ...
  useEffect(() => {
    ...
    fetch(`${API_URL}/merchant/submerchants/weight-disputes`, {
      method: 'GET',
      headers: {
      ...
      .then(result => {
        if (result.success) {
          result.data.rows.sort((a, b) => parseInt(a.dispute_id) - parseInt(b.dispute_id)).reverse();
          setReports(result.data.rows);
        } else {
...
const MySubmerchantWeightDisputes = () => {
  return (
...
export default MySubmerchantWeightDisputes
