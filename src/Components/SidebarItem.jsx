...
const SidebarItem = ({ item }) => {
    ...
    return (
        <>
            <div onClick={item.isDropdown ? () => setIsOpen(!isOpen) : (item.name === "Wallet Recharge" ? () => setShowRecharge(true) : () => navigate(item.url))} className={` cursor-pointer px-2 w-full h-12 ${isCurrentMenu ? 'bg-gray-600' : 'bg-gray-900'} focus:outline-none transition-all duration-300 hover:bg-gray-700 relative flex items-center`}>
